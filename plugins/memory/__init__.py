...
# Also scan user-installed memory plugins from ~/.hermes/plugins/memory/
_USER_MEMORY_PLUGINS_DIR = Path(
    os.environ.get("HERMES_HOME", Path.home() / ".hermes")
) / "plugins" / "memory"


def discover_memory_providers() -> List[Tuple[str, str, bool]]:
    """Scan both system and user memory plugin directories for available providers.

    Returns list of (name, description, is_available) tuples.
    Does NOT import the providers — just reads plugin.yaml for metadata
    and does a lightweight availability check.

    User plugins (~/.hermes/plugins/memory/) take priority over system plugins
    with the same name, and are checked in addition to system plugins.
    """
    results = []
    seen_names = set()

    # Scan system dir first, then user dir
    for plugins_dir in [_MEMORY_PLUGINS_DIR, _USER_MEMORY_PLUGINS_DIR]:
        ...
        for child in sorted(plugins_dir.iterdir()):
            ...
            if child.name in seen_names:
                continue  # User plugin overrides system plugin with same name
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    Checks user plugin directory (~/.hermes/plugins/memory/) first, then
    falls back to the system directory. Returns None if not found or fails to load.
    """
    # Check user dir first (allows override), then system dir
    for plugins_dir in [_USER_MEMORY_PLUGINS_DIR, _MEMORY_PLUGINS_DIR]:
        provider_dir = plugins_dir / name
        ...
        try:
            ...
            logger.warning("Failed to load memory provider '%s' from %s: %s", name, plugins_dir, e)
            return None

    logger.debug("Memory provider '%s' not found (checked %s and %s)",
                 name, _USER_MEMORY_PLUGINS_DIR, _MEMORY_PLUGINS_DIR)
    return None
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    if hasattr(mod, "register"):
        ...
        try:
            result = mod.register(collector)
            # register() may return the provider directly OR set collector.provider
