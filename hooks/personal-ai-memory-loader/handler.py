...
async def _fetch_memories() -> list[str]:
    ...
    Uses per-id asyncio.Event to correlate requests with responses — no more
    polling a shared queue that gets cleared between requests.
    """
    session_id = None
    # pending[id] -> asyncio.Event() — SSE reader sets event when matching id arrives
    pending: dict[str, asyncio.Event] = {}
    # responses[id] -> parsed JSON message — reader writes here, POST reads
    responses: dict[str, dict] = {}

    async def sse_reader(http: httpx.AsyncClient):
        """Reads SSE events and dispatches them to the right pending Event by id."""
        nonlocal session_id
        try:
    ...
    try:
        ...
        for mem_type in ("know", "policy"):
            ...
            await http.post(msg_url, json=payload, headers=_headers())

            # Wait up to 10s for this specific response by id
            for _ in range(100):
                if ev.is_set():
                    break
                await asyncio.sleep(0.1)

            msg_result = responses.get(msg_id)
            pending.pop(msg_id, None)
            responses.pop(msg_id, None)

            if msg_result and "result" in msg_result:
                content_list = msg_result["result"].get("content", [])
