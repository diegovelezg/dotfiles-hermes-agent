...
def _build_child_agent(
    ...
    # Inject parent's memory manager tool schemas into child agent.
    # This lets subagents use plugin tools (e.g. personal-ai-ledger:
    # ledger_query, ledger_item_create, browser_activity_query, etc.)
    # even when skip_memory=True.
    parent_mm = getattr(parent_agent, '_memory_manager', None)
    if parent_mm:
        try:
            for _schema in parent_mm.get_all_tool_schemas():
                _wrapped = {"type": "function", "function": _schema}
                _tname = _schema.get("name", "")
                if _tname and _tname not in child.valid_tool_names:
                    child.tools.append(_wrapped)
                    child.valid_tool_names.add(_tname)
        except Exception as _e:
            logger.debug("Could not inherit memory manager tools: %s", _e)
